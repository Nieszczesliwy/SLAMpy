...
def extractFeatures(image):
    ...
    points = cv2.goodFeaturesToTrack(
        np.mean(image, axis=2).astype(np.uint8), 1000, qualityLevel=0.01, minDistance=7)
    ...
    # Return keypoints and ORB descriptors
    return np.array([(kp.pt[0], kp.pt[1]) for kp in keypoints]), orb_descriptors

def normalizePoints(inverse_camera_matrix, points):
    return np.dot(inverse_camera_matrix, np.concatenate([points, np.ones((points.shape[0], 1))], axis=1).T).T[:, 0:2]
...
class Camera:
    def __init__(self, frame_registry, image, camera_matrix):
        ...
        self.camera_pose = identity_matrix
        ...
        self.camera_id = len(frame_registry.frames)
